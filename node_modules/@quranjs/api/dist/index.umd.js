...
(function (factory) {
  ...
  var { camelizeKeys, decamelize, decamelizeKeys } = import_humps.default;
  var makeUrl = (url, params) => {
    const baseUrl = `${API_BASE_URL}${removeBeginningSlash(url)}`;
    if (!params)
      return baseUrl;
    const paramsWithDecamelizedKeys = decamelizeKeys(params);
  ...
  var { decamelize: decamelize2 } = import_humps2.default;
  var defaultOptions2 = {
    language: "ar" /* ARABIC */,
    perPage: 50,
    words: false
  ...
  var mergeVerseOptions = (options = {}) => {
    ...
    if (result.wordFields) {
      ...
      Object.entries(result.wordFields).forEach(([key, value]) => {
        if (value)
          wordFields.push(decamelize2(key));
      });
    ...
    if (result.translationFields) {
      ...
      Object.entries(result.translationFields).forEach(([key, value]) => {
        if (value)
          translationFields.push(decamelize2(key));
      });
