...
const ProfileManager = {
    ...
    async loadSavedAyahs() {
        const container = document.getElementById('savedAyahsList');
        const countEl   = document.getElementById('ayahCount');
        const statusEl  = document.getElementById('ayahStatus');
        if (!container) return;
        ...
        try {
            ...
            const { data: ayahs, error } = await sb
                .from('saved_ayahs')
                .select('*')
                .eq('user_id', this.user.id)
                .order('created_at', { ascending: false });
            ...
            if (!ayahs || ayahs.length === 0) {
                container.innerHTML = `
                    <div class="text-center py-16 opacity-40">
                        ...
                        <p>لا توجد آيات محفوظة حالياً</p>
                    </div>`;
                if (countEl) countEl.innerText = '0';
                if (statusEl) statusEl.innerText = 'قائمة فارغة';
                return;
            ...
            if (countEl) countEl.innerText = ayahs.length;
            if (statusEl) statusEl.innerText = `${ayahs.length} آية`;

            container.innerHTML = ayahs.map(item => `
                <div id="ayah-${item.id}" class="glass-card rounded-[2rem] p-6 transition-all hover:border-teal-500/30 group mb-4">
                    <p class="quran-font text-xl text-right text-white/90 mb-3 leading-loose">
                        ${item.ayah_text || ''}
                    </p>
                    ${item.translation ? `<p class="text-sm text-slate-400 text-left mb-3 leading-relaxed border-t border-white/5 pt-3">${item.translation}</p>` : ''}
                    <div class="flex justify-between items-center border-t border-white/5 pt-4">
                        <button onclick="ProfileManager.removeSavedAyah(${item.id})"
                                class="text-xs text-red-400/50 hover:text-red-400 transition-colors flex items-center gap-2">
                            <i class="fas fa-trash-alt"></i> حذف
                        </button>
                        <div class="text-[10px] text-slate-500 font-bold tracking-widest">
                            سورة ${item.surah_number} &nbsp;•&nbsp; آية ${item.verse_number}
                        </div>
                    </div>
                </div>`).join('');
    ...
    async removeSavedAyah(id) {
        try {
            ...
            if (el) {
                ...
                setTimeout(() => this.loadSavedAyahs(), 300);
            }
            showToast('✅ تم الحذف');
        } catch (err) {
            console.error('Remove Ayah Error:', err);
            showToast('❌ فشل الحذف');
        }
    },
